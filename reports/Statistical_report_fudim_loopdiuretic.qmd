...
- National Dispensed Drug Register (The National Board of Health and Welfare) for XXX
...
in the Acknowledgments of any resulting articles. 
...
default_kable(flow) %>%
  row_spec(c(1, 10, 18, 34, 37), bold = T)
```
...
All analyses are performed for the loop diuretic exposure No loop diuretic, Dose 1-40mg/24h, Dose >40mg/24h.
...
The primary outcome CV mortality/first HF hospitalization was included as the Nelson-Aalen estimator.

## Baseline characteristics

Baseline characteristics were presented by loop diuretic
where categorical variables were presented with n and percentage and tested between groups with the chi-square test. 
Continuous variables were presented with median (q1-q3) and tested for differences between groups with the Kruskal-Wallis test. 
...
Crude and adjusted analyses were performed where adjustment was performed for variables indicated in @tbl-base. 
...
Data were censored at 3 years (OK??? Or should be longer/shorter follow-up????) follow-up, 2021-12-31, emigration from Sweden or, when it was itself not an event, death. 

### Assumptions

The proportional hazards assumption was investigated using the scaled Schoenfeld 
residuals (cox.zph in[@survival-package]) for the primary outcome leading to that location was stratified for in the models. 
Possible outliers were visually inspected by plotting the dfbetas. 

### Residual confounding

In order to assess how robust the association between loop diuretic and the primary outcome is 
to residual confounding the E-value 
was calculated[@evalue] [@evalue2]. The E-value is interpreted as the minimum Risk Ratio 
that an unmeasured confounder would need to have with both loop diuretic and 
the primary outcome, conditional on the variables currently adjusted 
for in the model, to fully eliminate the association between loop diuretic and the outcome. 
...
The median age (IQR) is `r med_pop1` and `r percfem_pop1`% females in Population 1 and 
