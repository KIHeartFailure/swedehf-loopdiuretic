...
for (i in seq_along(tabvars)) {
  tabtmp <- basetabfunc(tabvars[i], data = rsdata %>% filter(pop1), stratavar = "shf_loopdiuretic_cat", catdig = catdig, contdig = contdig)
...
for (i in seq_along(tabvars)) {
  tabtmp <- basetabfunc(tabvars[i], data = rsdata, stratavar = "shf_loopdiuretic_cat", catdig = catdig, contdig = contdig)
